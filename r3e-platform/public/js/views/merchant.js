...
async function toggleMEngine(on) {
  try {
    await API.toggleEngine(getMid(), on, R3E.user.email);
    showToast(on ? '🟢 Campaign engine activated!' : '🔴 Engine stopped.');
    renderMDashboard();
        ...
        <div>
          <div class="card" style="margin-bottom:14px">
            ...
            <div class="alert-info" style="margin-bottom:12px;font-size:12px">
              Get free credentials at <a href="https://developers.facebook.com/docs/whatsapp/cloud-api" target="_blank" style="color:var(--gold)">Meta Business Suite</a> — 1,000 free messages/month.
            </div>
...
async function sendManualWhatsApp(mid) {
  ...
  try {
    ...
    const d = await r.json();
    if (resultEl) resultEl.innerHTML = d.sent !== undefined
      ? '<div class="alert-success">Sent to ' + d.sent + ' customers' + (d.failed ? ', ' + d.failed + ' failed' : '') + '</div>'
      : '<div class="alert-error">' + (d.error||'Send failed') + '</div>';
  } catch(e) {
