...
async function openCreateUserModal(preRole, preSubRole) {
  ...
  if (preRole) selectUserRole(preRole, preSubRole||'');
}
...
function selectUserRole(role, subRole) {
  document.querySelectorAll('[id^="role-opt-"]').forEach(el => {
    ...
    if (inp && selected) inp.checked = true;
  });
...
async function submitCreateUser() {
  ...
  const sel    = document.querySelector('[name="nu-role"]:checked');

  if (!fname || !email || !pass || !sel)
    return showToast('Please fill all required fields and select a role.', 'error');
  if (pass.length < 8)
    return showToast('Password must be at least 8 characters.', 'error');

  const [userType, subRole] = sel.value.split('|');
...
async function submitEditUser(userId) {
  ...
  const sel   = document.getElementById('eu-role')?.value||'';
  const [userType, subRole] = sel.split('|');
  const btn = document.querySelector('.modal-footer .btn-primary');
