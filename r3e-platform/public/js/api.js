...
const API = {
  ...
  deleteUser:       (id, by)    => API.del(`/api/users/${id}`, { deletedBy: by }),
  updateUser:       (id, data)  => API.put(`/api/users/${id}`, data),
  ...
  toggleEngine: async (id, on, by) => {
    const r = await fetch(`/api/engine/toggle/${id}`, {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ on, triggeredBy: by })
    });
