...
const NAV = {
  superadmin: [
    ...
    { section:'Users',     items:[{ view:'users',          icon:'👥', label:'User Management'},
                                  { view:'sa-admins',     icon:'🛡️', label:'Admins'        },
...
const VIEW_MAP = {
  ...
  'users':        () => typeof renderSAUsers === 'function' ? renderSAUsers() : showToast('Loading...','info'),
...
async function toggleEngineAdmin(merchantId, on) {
  try {
    const result = await API.toggleEngine(merchantId, on, R3E.user.email);
    if (on && result.whatsapp) {
      const wa = result.whatsapp;
      if (wa.sent > 0)
        showToast('🟢 Engine ON! WhatsApp sent to ' + wa.sent + ' customers.', 'success');
      else if (wa.error)
        showToast('🟢 Engine ON — WA error: ' + wa.error, 'error');
      else
        showToast('🟢 Engine started! No customers to message yet.', 'success');
    } else {
      showToast('Engine ' + (on ? '🟢 started' : '🔴 stopped'), on ? 'success' : 'info');
    }
