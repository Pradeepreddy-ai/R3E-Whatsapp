...
/* POST /api/social/publish/:mid — auto-publish flyer using stored tokens */
app.post('/api/social/publish/:mid', wrap(async (req, res) => {
  ...
  const mid = req.params.merchantId || req.params.mid;

  const results = {};
  const accounts = await all(
    `SELECT * FROM social_accounts WHERE merchant_id=$1`, [mid]
  );
  const acctMap = {};
  accounts.forEach(a => acctMap[a.platform] = a);

  for (const platform of platforms) {
    const acct = acctMap[platform];
    if (!acct) { results[platform] = { success: false, error: 'Account not connected. Please connect first.' }; continue; }
...
`),
        ...
        const r = await fetch(`https://graph.facebook.com/v19.0/${acct.page_id}/photos`, {
          method: 'POST',
        ...
        if (d.id) {
          results[platform] = { success: true, postId: d.id, url: `https://www.facebook.com/${acct.page_id}/posts/${d.post_id||d.id}` };
        } else {
          results[platform] = { success: false, error: d.error?.error_user_msg || d.error?.message || 'Post failed' };
        }
      }

      /* ── Instagram ── */
      else if (platform === 'instagram') {
        /* Step 1: Upload image as FB CDN hosted media via page */
        const base64 = flyerDataUrl.replace(/^data:image\/\w+;base64,/, '');
        const imgBuf = Buffer.from(base64, 'base64');
        const boundary = 'R3EBoundary' + Date.now();
        const fbBody = Buffer.concat([
          Buffer.from(`--${boundary}
Content-Disposition: form-data; name="source"; filename="flyer.jpg"
Content-Type: image/jpeg
...
`),
        ...
        const uploadR = await fetch(`https://graph.facebook.com/v19.0/${acct.page_id}/photos`, {
          method: 'POST',
        ...
        if (!uploadD.id) {
          results[platform] = { success: false, error: 'Image upload failed: ' + (uploadD.error?.message||'Unknown') };
          continue;
        }

        /* Step 2: Create Instagram media container */
        const containerR = await fetch(
          `https://graph.facebook.com/v19.0/${acct.ig_user_id}/media`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            image_url: `https://www.facebook.com/photo/?fbid=${uploadD.id}`,
            caption, access_token: acct.access_token,
          })
        });
        const containerD = await containerR.json();
        if (!containerD.id) {
          results[platform] = { success: false, error: 'IG container failed: ' + (containerD.error?.message||'Unknown') };
          continue;
        }
        ...
        const pubR = await fetch(
          `https://graph.facebook.com/v19.0/${acct.ig_user_id}/media_publish`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ creation_id: containerD.id, access_token: acct.access_token })
        });
        const pubD = await pubR.json();
        if (pubD.id) {
          results[platform] = { success: true, postId: pubD.id };
        } else {
          results[platform] = { success: false, error: pubD.error?.message || 'Publish failed' };
        }
      }
      ...
      /* ── Google Business ── */
        ...
        if (acct.token_expires_at && new Date(acct.token_expires_at) < new Date() && acct.refresh_token) {
          ...
          if (rd.access_token) {
            ...
            await run(`UPDATE social_accounts SET access_token=$1,token_expires_at=$2 WHERE id=$3`,
              [token, new Date(Date.now() + (rd.expires_in||3600)*1000), acct.id]);
          }
        }

        const locR = await fetch(
          `https://mybusinessbusinessinformation.googleapis.com/v1/${acct.account_id}/locations?pageSize=1`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const locD = await locR.json();
        const location = locD.locations?.[0];
        if (!location) { results[platform] = { success: false, error: 'No Google Business location found.' }; continue; }
        ...
        const postR = await fetch(
          `https://mybusiness.googleapis.com/v4/${location.name}/localPosts`,
          {
            ...
            body: JSON.stringify({
              languageCode: 'en-GB',
              summary: caption || 'New offer available!',
              topicType: 'OFFER',
        ...
        if (postD.name) {
          results[platform] = { success: true, url: postD.searchUrl };
        } else {
          results[platform] = { success: false, error: postD.error?.message || 'Post failed' };
        }
      }
  ...
  await addLog('Social Publish', mid, '', Object.entries(results).map(([p,r])=>p+':'+r.success).join(', '));
  res.json({ results });
}));
